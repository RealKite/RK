		...
		<div id="boardList" class="table table-hover">
		 ...
		 <div class="greenTable outerTableFooter">
            <div class="tableFootStyle">
                <div class="links">
	                <a href="#" class="aTag" id="1">[처음]</a> 
                    <c:if test="${paging.curPage ne 1}">
                        <a href="#" class="aTag" id="${paging.prevPage}">[이전]</a> 
                    </c:if>
                    <c:forEach var="pageNum" begin="${paging.startPage}" end="${paging.endPage}">
                        <c:choose>
                            <c:when test="${pageNum eq paging.curPage}">
                                <span style="font-weight: bold;">
                                    <a href="#" class="aTag" id="${pageNum}" style="font-weight: bold; color:red;">
                                        ${pageNum}
                                    </a>
                                </span> 
                            </c:when>
                            <c:otherwise>
                                <a href="#" class="aTag" id="${pageNum}">${pageNum}</a> 
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${paging.curPage ne paging.pageCnt && paging.pageCnt > 0}">
                        <a href="#" class="aTag" id="${paging.nextPage}">[다음]</a> 
                    </c:if>
                    <c:if test="${paging.curRange ne paging.rangeCnt && paging.rangeCnt > 0}">
                        <a href="#" class="aTag" id="${paging.pageCnt}">[끝]</a> 
                    </c:if>
                </div>
            </div>
        </div>
